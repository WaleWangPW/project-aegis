...
function coverageHint(report) {
  ...
  if (isWaitingCurrentDayDaily(report)) return '缓存已到前一交易日，等 Tushare 发布今天日线';
...
function morningReadinessPanel(decision, stockSelection, fullYearCoverage, stockAgentCycle, rankingGate) {
  ...
  return `<section class="morning-readiness ${statusTone}" aria-label="明早可用状态">
    ...
    <div class="morning-next">
      <b>${coverageOk ? '下一步：继续验证策略稳定性' : coverageWaiting ? '下一步：收盘后自动重试今日日线' : '醒来后可批准：补当前过去一年全A股缓存'}</b>
...
function fullYearCoverageBoard(report) { if(!report) return `<div class="deep-board pending"><b>全A股一年覆盖</b><span>等待覆盖计划报告。当前不能声称已有过去一年全A股历史记录。</span></div>`; const cache=report.current_cache||{}; const target=report.target||{}; const plan=report.overnight_openclaw_plan||{}; const ok=report.answer_label==='YES'; const waiting=isWaitingCurrentDayDaily(report); const blockers=report.blockers||[]; const headline=ok?'已具备当前一年全市场候选缓存':waiting?'已覆盖到上一交易日，等待今日日线':'当前还不能说已有过去一年全A股记录'; const explanation=waiting?`目标区间 ${esc(target.target_start)} 到 ${esc(target.target_end)}。本地缓存已到 ${esc(cache.daily_end)}；${esc(target.target_end)} 的 Tushare 日线暂未发布，收盘后重试即可。`:`目标区间 ${esc(target.target_start)} 到 ${esc(target.target_end)}。本地缓存区间 ${esc(cache.daily_start)} 到 ${esc(cache.daily_end)}，因此当前只能作为历史样本/旧缓存证据，不能冒充最新一年全市场验证。`; return `<section class="strategy-evidence-snapshot full-year-board"><div><p class="eyebrow">全A股一年数据覆盖</p><h3>${headline}</h3><p>${explanation}</p></div><div class="snapshot-metrics"><article class="${ok?'snapshot-pass':'snapshot-veto'}"><span>结论</span><b>${esc(coverageHeadline(report))}</b><small>${esc(coverageHint(report))}</small></article><article><span>缓存交易日</span><b>${esc(cache.daily_file_count ?? 0)}</b><small>daily_by_trade_date</small></article><article><span>股票基础表</span><b>${esc(cache.stock_basic_row_count ?? 0)}</b><small>stock_basic rows</small></article><article><span>缓存日线行</span><b>${esc(cache.total_daily_rows ?? 0)}</b><small>历史横截面</small></article><article><span>建议批次</span><b>${esc(waiting?'收盘后重试':plan.estimated_batch_count ?? 0)}</b><small>${esc(waiting?'不放松 Gate':`${plan.recommended_batch_size_trade_dates ?? 0} 日/批`)}</small></article><article class="snapshot-veto"><span>推荐影响</span><b>0</b><small>需 gate 放行</small></article></div>${blockers.length?`<div class="ranking-gate-detail"><h4>${waiting?'当前阻塞点':'为什么不能直接用？'}</h4><div class="deep-result-strip">${blockers.slice(0,6).map(x=>`<small class="${waiting?'warn':'fail'}">${esc(x)}</small>`).join('')}</div></div>`:''}<p class="snapshot-boundary">OpenClaw 可高强度跑覆盖/沙盘验证，但只能只读或 simulation-only；不读密钥、不接券商、不下单、不调用交易 webhook。</p></section>`; }
