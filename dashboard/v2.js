...
function retryGuardEvidencePanel(guard, goalAudit) {
  const status = guard?.status || '未运行';
  const failed = guard?.retry_failure?.failed_dates || [];
  const firstFailure = failed[0];
  const pending = (goalAudit?.requirements || []).find(x => String(x.requirement || '').includes('current-day cache retry'));
  const toneClass = status === 'PASS' ? 'ok' : status === 'FAIL' ? 'warn' : 'hold';
  const title = status === 'PASS' ? 'A股补数受控执行完成' : status === 'FAIL' ? 'A股补数已尝试，等待数据源' : 'A股补数等待受控执行';
  const detail = status === 'FAIL'
    ? `${firstFailure?.trade_date || '今日'}：${firstFailure?.error || '数据源暂未返回足够日线'}`
    : status === 'PASS'
      ? `retry_exit_code=${guard.retry_exit_code}，audit_exit_code=${guard.audit_exit_code}`
      : '先通过 preflight，再运行 guarded runner。';
  return `<section class="evidence-alert ${toneClass}"><div><span>A股补数受控执行器</span><h3>${esc(title)}</h3><p>${esc(detail)}</p></div><div><b>${esc(status)}</b><small>${esc(pending?.pending || 'simulation-only；不接券商、不下单、不调用交易 webhook')}</small></div></section>`;
}
...
function retryGuardFailureText(guard) {
  const failed = guard?.retry_failure?.failed_dates || [];
  const first = failed[0] || {};
  if (first.trade_date || first.error) return `${first.trade_date || '今日'}：${first.error || '数据未返回'}`;
  return '补数链失败，等待数据源刷新';
}
function retryReadinessLabel(readiness, fallbackReport, guard) {
  if (guard?.status === 'PASS') return {label:'补数已跑', detail:`受控执行完成，exit ${guard.retry_exit_code}`};
  if (guard?.status === 'FAIL') return {label:'数据未出', detail:`受控重试失败：${retryGuardFailureText(guard)}`};
...
function dailyUseStatus(readiness) {
  const status = readiness?.status || 'UNKNOWN';
  if (status === 'READY_FOR_SIMULATION_USE') return {label:'可开始', tone:'ok', detail:'模拟研究可用'};
  if (status === 'READY_WITH_GUARDS') return {label:'可试用', tone:'hold', detail:'带护栏使用'};
  if (status === 'NOT_READY') return {label:'待修复', tone:'deny', detail:(readiness.blockers || []).slice(0,2).join(' / ') || 'readiness 阻塞'};
  return {label:'待生成', tone:'hold', detail:'运行 readiness 报告'};
}
function operationalModePanel(daily, dashboardIntent, fullYearCoverage, rankingGate, currentDayRetryReadiness, currentDayRetryGuard, dailyUseReadiness) {
  ...
  const retryStatus = retryReadinessLabel(currentDayRetryReadiness, fullYearCoverage, currentDayRetryGuard);
  const dailyUse = dailyUseStatus(dailyUseReadiness);
  const retryReady = currentDayRetryReadiness?.ready_to_run === true || retryStatus.label === '可以重试';
  ...
  const retryCopy = `make build-a-share-current-day-retry-readiness\nmake a-share-current-day-retry-guarded\n\n# Guarded runner only executes the raw chain after READY.\n# Equivalent explicit steps after READY:\n${retryCommand}\nmake build-a-share-full-year-coverage-plan\nmake stock-agent-a-share-strategy-cycle-managed-expanded`;
  ...
  return `<section class="operational-mode ${dailyFailed?'warn':'ok'}" aria-label="当前可用性解释"><div><span>当前使用模式</span><b>${esc(mode)}</b><p>${esc(modeText)}</p></div><div class="mode-grid"><article class="${esc(dailyUse.tone)}"><span>日常使用</span><b>${esc(dailyUse.label)}</b><small>${esc(dailyUse.detail)}</small></article><article class="ok"><span>页面操作</span><b>可用</b><small>多页面工作流</small></article><article class="${hasBridgeEvidence?'ok':'hold'}"><span>按钮回传</span><b>${hasBridgeEvidence?'已验证':'待点击'}</b><small>${hasBridgeEvidence?'后台 evidence 有记录':'点击后看回执'}</small></article><article class="${dailyFailed?'hold':'ok'}"><span>自动检查</span><b>${esc(stateLabel(dailyVerdict))}</b><small>${dailyFailed?'不等于页面不可用':'每日 08:00'}</small></article><article class="hold"><span>策略 Gate</span><b>${esc(gateApproved)}</b><small>放行前不排序推荐</small></article><article class="${retryReady?'ok action-ready':coverageWaiting?'hold action-ready':'ok action-ready'}"><span>A股补数</span><b>${esc(retryStatus.label)}</b><small>${esc(retryStatus.detail)}</small><div class="mode-actions"><button type="button" data-copy-text="${esc(retryCopy)}" data-copy-label="复制补数命令">复制补数命令</button><button type="button" data-page-jump="strategy">策略页</button></div></article><article class="deny"><span>真实交易</span><b>禁止</b><small>不接券商/不下单</small></article></div></section>`;
}
...
function morningReadinessPanel(decision, stockSelection, fullYearCoverage, stockAgentCycle, rankingGate, currentDayRetryReadiness, currentDayRetryGuard) {
  ...
  return `<section class="morning-readiness ${statusTone}" aria-label="明早可用状态">
    ...
    <div class="morning-grid">
      ...
      <article class="${retryStatus.label==='可以重试'?'ok':retryStatus.label==='已阻塞'?'deny':'hold'}"><span>重试预检</span><b>${esc(retryStatus.label)}</b><small>${esc(retryStatus.detail)}</small></article>
    </div>
...
function render() { const invalid=[]; return Promise.all(Object.entries(sources).map(async([key,file])=>[key,await read(file,invalid)])).then(entries=>{
  ...
  byId('overview-actions').innerHTML=operationalModePanel(d.daily,d.dashboardIntent,d.fullYearCoverage,d.rankingGate,d.currentDayRetryReadiness,d.currentDayRetryGuard,d.dailyUseReadiness)+morningReadinessPanel(decision,d.stockSelection,d.fullYearCoverage,d.stockAgentCycle,d.rankingGate,d.currentDayRetryReadiness,d.currentDayRetryGuard)+actionHub(decision,d.feedback,d.dashboardIntent);
  ...
  byId('evidence-content').innerHTML=`<p>证据检查：${tag(d.gate?.overall_verdict)}　数据流水线历史：${d.history?.runs?.length ?? '数据未提供'} 条记录　原始价格审计：${tag(d.audit?.overall_verdict)}</p>${retryGuardEvidencePanel(d.currentDayRetryGuard,d.goalAudit)}<ul class="detail-list">${reportLink('选股工作台数据',sources.stockSelection)}${reportLink('选股+资讯简报','stock_selection_news_digest_latest.md')}${reportLink('目标完成审计',sources.goalAudit)}${reportLink('Dashboard 日常可用性',sources.dailyUseReadiness)}${reportLink('Dashboard 日常 smoke',sources.dailyUseSmoke)}${reportLink('Dashboard 按钮 dry-run smoke',sources.intentDryRunSmoke)}${reportLink('Dashboard 真实点击验收',sources.realClickAcceptance)}${reportLink('A股 Tushare 策略源探测',sources.strategySourceProbe)}${reportLink('A股 source hypothesis 队列',sources.sourceHypotheses)}${reportLink('A股 source hypothesis 代理评估',sources.sourceHypothesisEvaluation)}${reportLink('A股龙虎榜/游资研究样本',sources.dragonTiger)}${reportLink('A股 source feature 覆盖',sources.sourceFeatureCoverage)}${reportLink('A股 source deep sandbox',sources.sourceDeepSandbox)}${reportLink('A股 refined strategy sandbox',sources.refinedSandbox)}${reportLink('A股 refined ranking gate',sources.rankingGate)}${reportLink('A股样本扩展计划',sources.sampleExpansion)}${reportLink('A股全市场一年覆盖计划',sources.fullYearCoverage)}${reportLink('A股当日补数预检',sources.currentDayRetryReadiness)}${reportLink('A股补数受控执行器',sources.currentDayRetryGuard)}${reportLink('A股策略诊断',sources.strategyDiagnostics)}${reportLink('stock-agent managed cycle',sources.stockAgentCycle)}${reportLink('策略验证输入','aegis_strategy_validation_input_latest.json')}${reportLink('策略沙盘覆盖',sources.strategySandbox)}${reportLink('逐标的历史 case',sources.strategyCases)}${reportLink('case 结果评估',sources.strategyCaseEvaluation)}${reportLink('Dashboard 本地按钮回执',sources.dashboardIntent)}${reportLink('最近按钮回传',sources.feedback)}${reportLink('当前模拟建议简报',sources.simulationBrief)}${reportLink('证据检查',sources.gate)}${reportLink('数据流水线历史',sources.history)}${reportLink('原始价格审计',sources.audit)}${reportLink('滚动回测历史',sources.rollingHistory)}${reportLink('信号快照','a_share_signal_snapshots_latest.json')}</ul><p class="muted">不可读取数据：${invalid.length?esc(invalid.length):'无'}</p>`;
