...
function localIntentBoard(feedback, compact=false) { const rows=localIntentLog(); const backend=feedback?.event; const backendText=backend?`${backend.symbol} · ${actionLabel(backend.action)} · ${backend.received_at}`:'后台暂未收到飞书/股票助手回传'; const latest=rows[0]; const submitted=latest?intentSubmitStatus(latest.symbol):null; const submitText=submitted?.status==='RECORDED'?'本地桥已写入':submitted?.status==='LOCAL_ONLY'?'仅本机记录':'等待点击'; return `<section class="intent-board ${compact?'compact':''}" aria-label="本机操作记录"><div class="intent-board-head"><div><span>操作记录</span><h3>${latest?`${esc(latest.symbol)} · ${esc(actionLabel(latest.action))}`:'还没有本机点击记录'}</h3><p>${latest?`本机已记录 ${esc(latest.time)}；${submitText==='本地桥已写入'?'后台 evidence 已自动写入。':'如果本地桥服务开启，会自动写入后台 evidence。'}`:'点击候选卡按钮后，这里会立刻显示本机记录。'}</p></div><div class="intent-buttons"><button class="copy-intents" type="button" data-copy-intents="${esc(intentExportText(rows))}">复制摘要</button><button class="copy-intents machine" type="button" data-copy-intents-json="${esc(intentExportPayload(rows))}">复制后台JSON</button></div></div><div class="intent-board-grid"><article><span>本机待确认</span><b>${esc(rows.length)}</b><small>localStorage</small></article><article><span>本地桥</span><b>${esc(submitText)}</b><small>${esc(submitted?.time||'serve-dashboard-intent-bridge')}</small></article><article><span>后台最新</span><b>${backend?'已收到':'未收到'}</b><small>${esc(backendText)}</small></article><article><span>交易边界</span><b>模拟</b><small>不接券商、不下单</small></article></div>${rows.length?`<ol class="intent-log">${rows.slice(0,5).map(x=>`<li><b>${esc(x.symbol)}</b><span>${esc(actionLabel(x.action))}</span><small>${esc(x.time)}</small></li>`).join('')}</ol>`:'<p class="empty intent-empty">暂无本机记录。你可以在选股页点“加入模拟研究 / 要更多资讯 / 暂不关注”。</p>'}</section>`; }
...
function actionHub(decision, feedback) {
  const latest = feedback?.event ? `${feedback.event.symbol} · ${feedback.event.action}` : '暂无新反馈';
...
function commandStrip(decision, stockSelection, fullYearCoverage, rankingGate, feedback) {
  ...
  const coverage = fullYearCoverage?.answer_label || 'NO';
...
function morningReadinessPanel(decision, stockSelection, fullYearCoverage, stockAgentCycle, rankingGate) {
  ...
  return `<section class="morning-readiness ${statusTone}" aria-label="明早可用状态">
    ...
    <div class="morning-grid">
      ...
      <article class="${coverageOk ? 'ok' : 'hold'}"><span>全A股一年</span><b>${coverageOk ? 'YES' : 'NO'}</b><small>${esc(fullYearCoverage?.coverage_status || '等待报告')}</small></article>
    ...
    <div class="morning-next">
      <b>${coverageOk ? '下一步：继续验证策略稳定性' : '醒来后可批准：补当前过去一年全A股缓存'}</b>
      <span>${coverageOk ? '仍需 ranking gate 放行后才允许影响排序。' : '未批准前不会消耗大量 Tushare API，也不会写入大缓存。'}</span>
    </div>
  </section>`;
}
...
function fullYearCoverageBoard(report) { if(!report) return `<div class="deep-board pending"><b>全A股一年覆盖</b><span>等待覆盖计划报告。当前不能声称已有过去一年全A股历史记录。</span></div>`; const cache=report.current_cache||{}; const target=report.target||{}; const plan=report.overnight_openclaw_plan||{}; const ok=report.answer_label==='YES'; const blockers=report.blockers||[]; return `<section class="strategy-evidence-snapshot full-year-board"><div><p class="eyebrow">全A股一年数据覆盖</p><h3>${ok?'已具备当前一年全市场候选缓存':'当前还不能说已有过去一年全A股记录'}</h3><p>目标区间 ${esc(target.target_start)} 到 ${esc(target.target_end)}。本地缓存区间 ${esc(cache.daily_start)} 到 ${esc(cache.daily_end)}，因此当前只能作为历史样本/旧缓存证据，不能冒充最新一年全市场验证。</p></div><div class="snapshot-metrics"><article class="${ok?'snapshot-pass':'snapshot-veto'}"><span>结论</span><b>${esc(report.answer_label)}</b><small>${esc(report.coverage_status)}</small></article><article><span>缓存交易日</span><b>${esc(cache.daily_file_count ?? 0)}</b><small>daily_by_trade_date</small></article><article><span>股票基础表</span><b>${esc(cache.stock_basic_row_count ?? 0)}</b><small>stock_basic rows</small></article><article><span>缓存日线行</span><b>${esc(cache.total_daily_rows ?? 0)}</b><small>历史横截面</small></article><article><span>建议批次</span><b>${esc(plan.estimated_batch_count ?? 0)}</b><small>${esc(plan.recommended_batch_size_trade_dates ?? 0)} 日/批</small></article><article class="snapshot-veto"><span>推荐影响</span><b>0</b><small>需 gate 放行</small></article></div>${blockers.length?`<div class="ranking-gate-detail"><h4>为什么不能直接用？</h4><div class="deep-result-strip">${blockers.slice(0,6).map(x=>`<small class="fail">${esc(x)}</small>`).join('')}</div></div>`:''}<p class="snapshot-boundary">OpenClaw 可高强度跑覆盖/沙盘验证，但只能只读或 simulation-only；不读密钥、不接券商、不下单、不调用交易 webhook。</p></section>`; }
...
function render() { const invalid=[]; return Promise.all(Object.entries(sources).map(async([key,file])=>[key,await read(file,invalid)])).then(entries=>{
  ...
  byId('overview-actions').innerHTML=morningReadinessPanel(decision,d.stockSelection,d.fullYearCoverage,d.stockAgentCycle,d.rankingGate)+actionHub(decision,d.feedback);
  ...
  byId('local-intents-content').innerHTML=localIntentBoard(d.feedback);
  ...
  byId('evidence-content').innerHTML=`<p>证据检查：${tag(d.gate?.overall_verdict)}　数据流水线历史：${d.history?.runs?.length ?? '数据未提供'} 条记录　原始价格审计：${tag(d.audit?.overall_verdict)}</p><ul class="detail-list">${reportLink('选股工作台数据',sources.stockSelection)}${reportLink('选股+资讯简报','stock_selection_news_digest_latest.md')}${reportLink('A股 Tushare 策略源探测',sources.strategySourceProbe)}${reportLink('A股 source hypothesis 队列',sources.sourceHypotheses)}${reportLink('A股 source hypothesis 代理评估',sources.sourceHypothesisEvaluation)}${reportLink('A股龙虎榜/游资研究样本',sources.dragonTiger)}${reportLink('A股 source feature 覆盖',sources.sourceFeatureCoverage)}${reportLink('A股 source deep sandbox',sources.sourceDeepSandbox)}${reportLink('A股 refined strategy sandbox',sources.refinedSandbox)}${reportLink('A股 refined ranking gate',sources.rankingGate)}${reportLink('A股样本扩展计划',sources.sampleExpansion)}${reportLink('A股全市场一年覆盖计划',sources.fullYearCoverage)}${reportLink('A股策略诊断',sources.strategyDiagnostics)}${reportLink('stock-agent managed cycle',sources.stockAgentCycle)}${reportLink('策略验证输入','aegis_strategy_validation_input_latest.json')}${reportLink('策略沙盘覆盖',sources.strategySandbox)}${reportLink('逐标的历史 case',sources.strategyCases)}${reportLink('case 结果评估',sources.strategyCaseEvaluation)}${reportLink('最近按钮回传',sources.feedback)}${reportLink('当前模拟建议简报',sources.simulationBrief)}${reportLink('证据检查',sources.gate)}${reportLink('数据流水线历史',sources.history)}${reportLink('原始价格审计',sources.audit)}${reportLink('滚动回测历史',sources.rollingHistory)}${reportLink('信号快照','a_share_signal_snapshots_latest.json')}</ul><p class="muted">不可读取数据：${invalid.length?esc(invalid.length):'无'}</p>`;
