...
function operationalModePanel(daily, dashboardIntent, fullYearCoverage, rankingGate, currentDayRetryReadiness, dailyUseReadiness) {
  ...
  const retryStatus = retryReadinessLabel(currentDayRetryReadiness, fullYearCoverage);
...
function morningReadinessPanel(decision, stockSelection, fullYearCoverage, stockAgentCycle, rankingGate, currentDayRetryReadiness) {
...
function render() { const invalid=[]; return Promise.all(Object.entries(sources).map(async([key,file])=>[key,await read(file,invalid)])).then(entries=>{
  ...
  byId('evidence-content').innerHTML=`<p>证据检查：${tag(d.gate?.overall_verdict)}　数据流水线历史：${d.history?.runs?.length ?? '数据未提供'} 条记录　原始价格审计：${tag(d.audit?.overall_verdict)}</p><ul class="detail-list">${reportLink('选股工作台数据',sources.stockSelection)}${reportLink('选股+资讯简报','stock_selection_news_digest_latest.md')}${reportLink('目标完成审计',sources.goalAudit)}${reportLink('Dashboard 日常可用性',sources.dailyUseReadiness)}${reportLink('Dashboard 日常 smoke',sources.dailyUseSmoke)}${reportLink('Dashboard 按钮 dry-run smoke',sources.intentDryRunSmoke)}${reportLink('Dashboard 真实点击验收',sources.realClickAcceptance)}${reportLink('A股 Tushare 策略源探测',sources.strategySourceProbe)}${reportLink('A股 source hypothesis 队列',sources.sourceHypotheses)}${reportLink('A股 source hypothesis 代理评估',sources.sourceHypothesisEvaluation)}${reportLink('A股龙虎榜/游资研究样本',sources.dragonTiger)}${reportLink('A股 source feature 覆盖',sources.sourceFeatureCoverage)}${reportLink('A股 source deep sandbox',sources.sourceDeepSandbox)}${reportLink('A股 refined strategy sandbox',sources.refinedSandbox)}${reportLink('A股 refined ranking gate',sources.rankingGate)}${reportLink('A股样本扩展计划',sources.sampleExpansion)}${reportLink('A股全市场一年覆盖计划',sources.fullYearCoverage)}${reportLink('A股当日补数预检',sources.currentDayRetryReadiness)}${reportLink('A股策略诊断',sources.strategyDiagnostics)}${reportLink('stock-agent managed cycle',sources.stockAgentCycle)}${reportLink('策略验证输入','aegis_strategy_validation_input_latest.json')}${reportLink('策略沙盘覆盖',sources.strategySandbox)}${reportLink('逐标的历史 case',sources.strategyCases)}${reportLink('case 结果评估',sources.strategyCaseEvaluation)}${reportLink('Dashboard 本地按钮回执',sources.dashboardIntent)}${reportLink('最近按钮回传',sources.feedback)}${reportLink('当前模拟建议简报',sources.simulationBrief)}${reportLink('证据检查',sources.gate)}${reportLink('数据流水线历史',sources.history)}${reportLink('原始价格审计',sources.audit)}${reportLink('滚动回测历史',sources.rollingHistory)}${reportLink('信号快照','a_share_signal_snapshots_latest.json')}</ul><p class="muted">不可读取数据：${invalid.length?esc(invalid.length):'无'}</p>`;
