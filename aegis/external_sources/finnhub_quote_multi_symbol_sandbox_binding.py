"""Bind multi-symbol Finnhub quote research context to sandbox candidates.

V2.13-T creates sandbox candidate packets for each verified V2.13-S research
context item. It does not assemble historical cases, run sandbox evaluation, or
enable suggestions.
"""
...
SOURCE_ACCEPTANCE_TARGET = "V2.13-S Finnhub Quote Multi-Symbol Research Context Bridge"
...
def _candidate_for_context(context: Mapping[str, Any], *, created_at: str) -> StrategyCandidate:
    ...
    return StrategyCandidate(
        strategy_id=f"strategy_{symbol}_finnhub_multi_quote_context_probe",
        name=f"{context['canonical_symbol']} Finnhub multi-symbol quote context sandbox probe",
        ...
        risk_controls=[
            ...
            "multi_symbol_context_not_cross_symbol_signal",
        ],
        ...
        source_research_refs=[
            ...
            str(context["source_quote_json_sha256"]),
            str(context.get("source_artifact_ref_sha256")),
        ],
...
def _binding_for_context(context: Mapping[str, Any], *, created_at: str) -> dict[str, Any]:
    ...
    return {
        "binding_id": f"bind_{context['context_id']}_multi_symbol_sandbox_candidate",
        ...
        "source_artifact_ref_sha256": context.get("source_artifact_ref_sha256"),
...
def build_finnhub_quote_multi_symbol_sandbox_binding_report(
    ...
    *,
    ...
    source_symbols = sorted(str(symbol) for symbol in source_report.get("summary", {}).get("symbols", []))
    binding_symbols = sorted(str(item["canonical_symbol"]) for item in bindings)
    checks = {
        ...
        "source_context_item_count_at_least_three": int(source_report.get("summary", {}).get("context_item_count") or 0)
        >= 3,
        "binding_symbols_match_source_symbols": binding_symbols == source_symbols and bool(binding_symbols),
        ...
        "at_least_three_bindings_created": len(bindings) >= 3,
        ...
        "all_strategy_candidates_have_pass_criteria": bool(bindings)
        and all(item["strategy_candidate"].get("pass_criteria") for item in bindings),
        ...
        "simulation_only": all(item["simulation_only"] is True for item in bindings),
        "manual_external_execution_only": all(item["manual_external_execution_only"] is True for item in bindings),
        ...
        "no_real_trade": all(item["no_real_trade"] is True for item in bindings),
        "no_broker_api": all(item["no_broker_api"] is True for item in bindings),
        "no_webhook": all(item["no_webhook"] is True for item in bindings),
        "no_order_placement": all(item["no_order_placement"] is True for item in bindings),
        "no_position_size": all(item["no_position_size"] is True for item in bindings),
        "no_live_order_signal": all(item["no_live_order_signal"] is True for item in bindings),
...
def render_finnhub_quote_multi_symbol_sandbox_binding_markdown(report: Mapping[str, Any]) -> str:
    lines = [
    ...
    for item in report.get("bindings", []) or []:
        ...
        lines.extend(
            [
                ...
                "",
                f"- binding_id: `{item.get('binding_id')}`",
