"""Feedback intake for V2.12-F H/US simulation brief.

The intake records user reactions to simulation-only H/US brief items. It can
create follow-up evidence candidates for later paper simulation review, but it
never mutates PaperTrade, Recommendation, Review, Memory, broker state, webhook
state, or orders.
"""
...
def build_h_us_brief_feedback_records(
...
def build_h_us_simulation_followup_candidates(records: Sequence[Mapping], *, evidence_ref: str) -> list[dict]:
    ...
    for record in records:
        ...
        candidates.append(
            {
                "followup_id": f"h_us_feedback_followup_{record['feedback_id']}",
...
def build_h_us_brief_feedback_intake_report(
    feedback_inputs: Sequence[ManualFeedbackInput | Mapping],
    *,
    brief: Mapping,
    run_id: str,
    ...
    records = build_h_us_brief_feedback_records(
        feedback_inputs,
        brief=brief,
        evidence_ref=evidence_ref,
        screenshot_root=screenshot_root,
    )
    followups = build_h_us_simulation_followup_candidates(records, evidence_ref=evidence_ref)
    ...
    checks = {
        "source_is_v2_12_f": brief.get("acceptance_target")
    ...
    return {
        ...
        "summary": {
            ...
            "next_stage": "V2.12-H H-US Feedback To Paper Simulation Review Queue",
        },
        "records": records,
