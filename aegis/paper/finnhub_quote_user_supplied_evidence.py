"""Validate user-supplied Finnhub quote paper simulation evidence.

This module turns pending Finnhub quote review queue items into virtual
PaperTrade creation candidates only after user-provided price, date, evidence,
explicit simulation confirmation, and explicit review confirmation are present.
It still does not write PaperTrade, Recommendation, Review, or Memory records.
"""
...
def validate_finnhub_quote_user_supplied_evidence(
    ...
    *,
    ...
    for item in user_evidence_inputs:
        ...
        if item.get("explicit_review_before_paper_trade") is not True:
            reasons.append("missing_explicit_review_before_paper_trade")
        ...
        base = {
            ...
            "explicit_review_before_paper_trade": item.get("explicit_review_before_paper_trade") is True,
        ...
        validated.append(
            {
                **base,
                ...
                "virtual_paper_trade_create_candidate_id": f"finnhub_quote_paper_create_candidate_{queue_id}",
...
def build_finnhub_quote_user_supplied_evidence_report(
    ...
    *,
    ...
    result = validate_finnhub_quote_user_supplied_evidence(review_queue, user_evidence_inputs)
    validated = result["validated_user_evidence_records"]
    blocked = result["blocked_user_evidence_records"]
    checks = {
        "source_is_v2_13_k": source_report.get("acceptance_target")
        ...
        "explicit_review_confirmation_required": all(
            item["explicit_review_before_paper_trade"] is True for item in validated
        ),
        ...
        "no_live_price": all(item["no_live_price"] for item in [*validated, *blocked]),
        "no_position_size": all(item["no_position_size"] for item in [*validated, *blocked]),
        "no_live_order_signal": all(item["no_live_order_signal"] for item in [*validated, *blocked]),
