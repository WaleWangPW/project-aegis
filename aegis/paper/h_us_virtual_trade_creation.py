"""Create H/US virtual PaperTrade ledger records from validated evidence.
...
def build_h_us_virtual_paper_trades(
    ...
    *,
    ...
    for item in validated_user_evidence_records:
        trade = PaperTrade(
            paper_trade_id=f"h_us_ptr_virtual_{_safe_id(item['queue_id'])}",
            recommendation_id=f"h_us_manual_entry_{_safe_id(item['queue_id'])}",
            symbol=item["symbol"],
            market=item["market"],
            direction="long",
...
def build_h_us_virtual_trade_creation_report(
    ...
    *,
    ...
    trades = build_h_us_virtual_paper_trades(validated)
    checks = {
        "source_is_v2_12_i": source_report.get("acceptance_target")
    ...
    return {
        ...
        "acceptance_target": "V2.12-J H-US Virtual PaperTrade Creation From Validated Evidence",
