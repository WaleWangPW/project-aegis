#!/usr/bin/env python3
"""Validate V2.12-I H/US user-supplied paper evidence."""
...
from aegis.paper.h_us_user_supplied_evidence import (  # noqa: E402
    build_h_us_user_supplied_evidence_report,
)

REPORTS_DIR = ROOT / "data" / "reports"
OUTPUT_ROOT = ROOT / "data" / "processed" / "v2_12_i_acceptance"
SOURCE_V2_12_H_REPORT_JSON = REPORTS_DIR / "v2_12_h_h_us_feedback_review_queue_latest.json"
SOURCE_V2_12_H_PASS_MARKER = REPORTS_DIR / "V2_12_H_H_US_FEEDBACK_REVIEW_QUEUE_PASS.marker"
...
PASS_MARKER = "V2_12_I_H_US_USER_SUPPLIED_PAPER_EVIDENCE_PASS.marker"
FAIL_MARKER = "V2_12_I_H_US_USER_SUPPLIED_PAPER_EVIDENCE_FAIL.marker"
REPORT_JSON = "v2_12_i_h_us_user_supplied_paper_evidence_latest.json"
REPORT_MD = "v2_12_i_h_us_user_supplied_paper_evidence_latest.md"
...
def _fixture_user_evidence_inputs(source_report: dict, run_dir: Path) -> list[dict]:
    ...
    if len(queue) < 2:
        raise ValueError("V2.12-I fixture requires at least two H/US review queue items")

    evidence_dir = run_dir / "user_supplied_h_us_evidence"
    evidence_dir.mkdir(parents=True, exist_ok=True)
    valid_evidence = evidence_dir / f"{queue[0]['symbol'].replace('.', '_')}_entry_evidence.txt"
    valid_evidence.write_text(
        f"user supplied H/US simulated entry evidence for {queue[0]['symbol']}\n",
        encoding="utf-8",
    )
    ...
    return [
        {
            ...
            "notes": "user manually confirmed simulated entry evidence",
        },
        {
            ...
            "evidence_refs": [],
            "notes": "blocked fixture missing user price evidence and confirmation",
        },
    ]
...
def run_acceptance(
    *,
    ...
    run_id: str = "v2_12_i_20260712_acceptance",
    command: str | None = None,
    source_v2_12_h_report_json: Path = SOURCE_V2_12_H_REPORT_JSON,
    source_v2_12_h_pass_marker: Path = SOURCE_V2_12_H_PASS_MARKER,
    ...
    source_report = _load_json(source_v2_12_h_report_json)
    if user_evidence_inputs_json is None:
        user_inputs = _fixture_user_evidence_inputs(source_report, run_dir)
        source_user_inputs_json = run_dir / "h_us_user_supplied_evidence_inputs.fixture.json"
    ...
    validated_json = run_dir / "validated_h_us_user_evidence_records.json"
    blocked_json = run_dir / "blocked_h_us_user_evidence_records.json"
    candidates_json = run_dir / "h_us_virtual_paper_trade_create_candidates.json"
    ...
    checks = {
        **report["checks"],
        ...
        == "V2.12-I H-US User-Supplied Paper Evidence Validation",
        "report_status_pass": report["overall_status"] == "PASS",
        "source_marker_exists": source_v2_12_h_pass_marker.exists(),
    ...
    report["source_v2_12_h_report_json"] = str(source_v2_12_h_report_json)
    report["source_v2_12_h_pass_marker"] = str(source_v2_12_h_pass_marker)
    ...
    report["hashes"] = {
        "source_v2_12_h_report_json": _sha256(source_v2_12_h_report_json),
        "source_v2_12_h_pass_marker": _sha256(source_v2_12_h_pass_marker),
    ...
    (reports_dir / marker_name).write_text(
        "\n".join(
            [
                ...
                "target=V2.12-I H-US User-Supplied Paper Evidence Validation",
                ...
                f"source_v2_12_h_report_json={source_v2_12_h_report_json}",
                f"source_v2_12_h_report_json_sha256={_sha256(source_v2_12_h_report_json)}",
                f"validated_user_evidence_count={report['summary']['validated_user_evidence_count']}",
                f"blocked_user_evidence_count={report['summary']['blocked_user_evidence_count']}",
                "network_used=false",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--run-id", default="v2_12_i_20260712_acceptance")
    parser.add_argument("--source-v2-12-h-report-json", type=Path, default=SOURCE_V2_12_H_REPORT_JSON)
    parser.add_argument("--source-v2-12-h-pass-marker", type=Path, default=SOURCE_V2_12_H_PASS_MARKER)
    ...
    report = run_acceptance(
        ...
        run_id=args.run_id,
        command=command,
        source_v2_12_h_report_json=args.source_v2_12_h_report_json,
        source_v2_12_h_pass_marker=args.source_v2_12_h_pass_marker,
