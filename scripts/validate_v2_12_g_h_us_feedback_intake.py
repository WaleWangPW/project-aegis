#!/usr/bin/env python3
"""Validate V2.12-G H/US user feedback intake for simulation brief."""
...
from aegis.feedback.h_us_brief_intake import build_h_us_brief_feedback_intake_report  # noqa: E402

REPORTS_DIR = ROOT / "data" / "reports"
OUTPUT_ROOT = ROOT / "data" / "processed" / "v2_12_g_acceptance"
SOURCE_V2_12_F_BRIEF_JSON = REPORTS_DIR / "v2_12_f_h_us_current_simulation_brief_latest.json"
SOURCE_V2_12_F_PASS_MARKER = REPORTS_DIR / "V2_12_F_H_US_CURRENT_SIMULATION_BRIEF_PASS.marker"
...
PASS_MARKER = "V2_12_G_H_US_FEEDBACK_INTAKE_PASS.marker"
FAIL_MARKER = "V2_12_G_H_US_FEEDBACK_INTAKE_FAIL.marker"
REPORT_JSON = "v2_12_g_h_us_feedback_intake_latest.json"
REPORT_MD = "v2_12_g_h_us_feedback_intake_latest.md"
...
def _fixture_feedbacks(brief: dict, screenshot_path: Path) -> list[dict]:
    ...
    if len(items) < 2:
        raise ValueError("V2.12-G fixture requires at least two H/US simulation candidate items")
    h_item = next((item for item in items if item.get("market") == "H"), items[0])
    us_item = next((item for item in items if item.get("market") == "US"), items[-1])
    return [
        {
            "feedback_id": "h_us_fb_watch_001",
            "suggestion_id": h_item["suggestion_id"],
            "symbol": h_item["symbol"],
            "market": h_item["market"],
            "feedback_type": "manual_watch",
            "user_note": "加入外部观察清单，等待人工核对实时价格和事件。",
            "screenshot_paths": [str(screenshot_path)],
            "submitted_at": "2026-07-12T01:00:00+08:00",
        },
        {
            "feedback_id": "h_us_fb_ignore_002",
            "suggestion_id": us_item["suggestion_id"],
            "symbol": us_item["symbol"],
            "market": us_item["market"],
            "feedback_type": "manual_ignore",
            "user_note": "暂时忽略，样本太小，先不进入观察。",
            "screenshot_paths": [],
            "submitted_at": "2026-07-12T01:00:00+08:00",
        },
        {
            ...
            "external_execution_summary": f"外部手动记录 {us_item['symbol']}，不是 Aegis 下单。",
            "screenshot_paths": [],
            "submitted_at": "2026-07-12T01:00:00+08:00",
        },
        {
            "feedback_id": "h_us_fb_unknown_004",
        ...
        {
            "feedback_id": "h_us_fb_secret_005",
            "suggestion_id": h_item["suggestion_id"],
            "symbol": h_item["symbol"],
            "market": h_item["market"],
            "feedback_type": "manual_watch",
...
def run_acceptance(
    *,
    ...
    run_id: str = "v2_12_g_20260712_acceptance",
    command: str | None = None,
    source_v2_12_f_brief_json: Path = SOURCE_V2_12_F_BRIEF_JSON,
    source_v2_12_f_pass_marker: Path = SOURCE_V2_12_F_PASS_MARKER,
    ...
    brief = _load_json(source_v2_12_f_brief_json)
    screenshot = run_dir / "fixture_h_us_feedback_screenshot.txt"
    screenshot.write_text("fixture H/US feedback screenshot without secrets\n", encoding="utf-8")
    feedback_inputs = _fixture_feedbacks(brief, screenshot)
    feedback_inputs_json = run_dir / "h_us_feedback_inputs.json"
    ...
    report = build_h_us_brief_feedback_intake_report(
        ...
        brief=brief,
        run_id=run_id,
        evidence_ref=str(source_v2_12_f_brief_json),
        command=command,
    )
    records_json = run_dir / "h_us_feedback_records.json"
    followups_json = run_dir / "h_us_simulation_followup_candidates.json"
    ...
    checks = {
        **report["checks"],
        ...
        "source_marker_exists": source_v2_12_f_pass_marker.exists(),
    ...
    report["source_v2_12_f_brief_json"] = str(source_v2_12_f_brief_json)
    report["source_v2_12_f_pass_marker"] = str(source_v2_12_f_pass_marker)
    ...
    report["hashes"] = {
        "source_v2_12_f_brief_json": _sha256(source_v2_12_f_brief_json),
        "source_v2_12_f_pass_marker": _sha256(source_v2_12_f_pass_marker),
    ...
    (reports_dir / marker_name).write_text(
        "\n".join(
            [
                ...
                "target=V2.12-G H-US User Feedback Intake For Simulation Brief",
                ...
                f"source_v2_12_f_brief_json={source_v2_12_f_brief_json}",
                f"source_v2_12_f_brief_json_sha256={_sha256(source_v2_12_f_brief_json)}",
                f"feedback_count={report['summary']['feedback_count']}",
                f"accepted_count={report['summary']['accepted_count']}",
                f"blocked_count={report['summary']['blocked_count']}",
                f"simulation_followup_count={report['summary']['simulation_followup_count']}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--run-id", default="v2_12_g_20260712_acceptance")
    parser.add_argument("--source-v2-12-f-brief-json", type=Path, default=SOURCE_V2_12_F_BRIEF_JSON)
    parser.add_argument("--source-v2-12-f-pass-marker", type=Path, default=SOURCE_V2_12_F_PASS_MARKER)
    ...
    report = run_acceptance(
        ...
        run_id=args.run_id,
        command=command,
        source_v2_12_f_brief_json=args.source_v2_12_f_brief_json,
        source_v2_12_f_pass_marker=args.source_v2_12_f_pass_marker,
    )
    print(
        "V2.12-G H-US User Feedback Intake For Simulation Brief",
