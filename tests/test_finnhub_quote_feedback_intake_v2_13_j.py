...
from aegis.feedback.finnhub_quote_brief_intake import build_finnhub_quote_feedback_intake_report
import scripts.validate_v2_13_j_finnhub_quote_feedback_intake as validator
        ...
        "items": [
            {
                ...
                "evidence_refs": ["source-report.json", "v2_13_f_quote_context_case:hash"],
            }
        ],
    }
...
def test_v2_13_j_feedback_intake_accepts_valid_feedback_and_blocks_risky_inputs(tmp_path: Path):
    ...
    assert all(item["requires_user_price_before_paper_trade"] for item in report["simulation_followup_candidates"])
    assert all(item["requires_user_date_before_paper_trade"] for item in report["simulation_followup_candidates"])


def test_v2_13_j_fails_if_source_is_not_v2_13_i(tmp_path: Path):
    screenshot = tmp_path / "screen.txt"
    screenshot.write_text("screenshot placeholder\n", encoding="utf-8")
    brief = _brief()
    ...
    assert report["checks"]["source_is_v2_13_i"] is False


def test_v2_13_j_fails_if_social_sentiment_not_blocked(tmp_path: Path):
    screenshot = tmp_path / "screen.txt"
    screenshot.write_text("screenshot placeholder\n", encoding="utf-8")
    brief = _brief()
    brief["summary"]["social_sentiment_status"] = "pass"
...
def test_v2_13_j_validator_writes_outputs_and_preserves_records(tmp_path: Path):
    ...
    report = validator.run_acceptance(
        ...
        source_v2_13_i_brief_json=brief_json,
        source_v2_13_i_pass_marker=marker,
...
def test_v2_13_j_cli_exits_zero_and_prints_no_secret_values(tmp_path: Path, capsys):
    brief_json = tmp_path / "brief.json"
    marker = tmp_path / "brief.marker"
    _write_json(brief_json, _brief())
    marker.write_text("exit_code=0\n", encoding="utf-8")
